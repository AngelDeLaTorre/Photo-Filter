...
int main( int argc, const char** argv )
{
    if (argc < 5) {
        ...
        printf("Specifying gaussian will apply the gaussian blur\n");
    ...
    // save the image.
    cvSaveImage(outputPath, result);
    cvReleaseImage(&image);
    cvReleaseImage(&result);

//    Code to see the resulting image in a window.
//    namedWindow("MyWindow", CV_WINDOW_AUTOSIZE); //create a window with the name "MyWindow"
//    cvShowImage("MyWindow", result); //display the image which is stored in the 'img' in the "MyWindow" window
